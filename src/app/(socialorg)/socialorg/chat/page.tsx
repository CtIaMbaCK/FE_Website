                ...
                searchResults.map((user) => (
                  ...
                  >
                    <div className="flex items-center gap-4 w-full">
                      {/* Avatar */}
                      <div className={`w-12 h-12 rounded-full ${user.role === 'ADMIN' ? 'bg-gradient-to-br from-red-400 to-red-600' : 'bg-gradient-to-br from-teal-400 to-[#008080]'} text-white flex items-center justify-center font-bold text-lg flex-shrink-0 shadow-sm group-hover:shadow transition-all`}>
                        {user.role === 'ADMIN' ? 'A' : getDisplayName(user)[0]}
                      </div>
            ...
            <>
              {conversations.length === 0 ? (
                ...
                conversations.map((conv) => {
                  // Tin nhắn chưa đọc từ người khác 
                  const isUnread =
                    conv.lastMessage &&
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-4">
                        {/* Avatar */}
                        <div className={`w-12 h-12 rounded-full ${conv.otherUser.role === 'ADMIN' ? 'bg-red-100 text-red-600' : 'bg-gradient-to-br from-slate-200 to-slate-300 text-slate-600'} flex items-center justify-center font-black text-xl flex-shrink-0 shadow-sm relative`}>
                          {conv.otherUser.role === 'ADMIN' ? 'A' : getDisplayName(conv.otherUser)[0]}
                          {isUnread && (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            <h3
                              className={`truncate ${isUnread ? "font-black text-slate-900" : "font-bold text-slate-700"} flex-1`}
                            >
        ...
        {selectedConversation ? (
          <>
            ...
            <div className="p-4 px-8 border-b border-slate-100/60 bg-white/50 backdrop-blur flex items-center justify-between sticky top-0 z-10">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-full ${selectedConversation.otherUser.role === 'ADMIN' ? 'bg-gradient-to-br from-red-400 to-red-600' : 'bg-gradient-to-br from-teal-400 to-[#008080]'} text-white flex items-center justify-center font-black text-xl shadow-md`}>
                  {selectedConversation.otherUser.role === 'ADMIN' ? 'A' : getDisplayName(selectedConversation.otherUser)[0]}
                </div>
                <div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8 text-center bg-slate-50/30">
            ...
            <p className="text-sm font-medium text-slate-500 max-w-sm">
              Nhấn vào một hội thoại từ danh sách hoặc dùng ô tìm kiếm để bắt đầu nhắn tin!
            </p>
          </div>
        )}
      </div>
